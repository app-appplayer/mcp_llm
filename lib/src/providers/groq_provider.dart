...
/// Implementation of LLM interface for Groq API (OpenAI-compatible).
/// Groq provides fast inference for open-source models like Llama and Mixtral.
class GroqProvider implements LlmInterface, RetryableLlmProvider {
  @override
  ...
  @override
  final Logger logger = Logger('mcp_llm.groq_provider');
  ...
  /// Groq does not expose a prompt-caching mechanism.
  @override
  ...
  Future<LlmResponse> complete(LlmRequest request) async {
    return await executeWithRetry(() async {
      logger.debug('Groq complete request with model: $model');
      ...
      if (httpResponse.statusCode >= 200 && httpResponse.statusCode < 300) {
        ...
        final error = 'Groq API Error: ${httpResponse.statusCode} - ${httpResponse.body}';
        logger.error(error);
  ...
  Stream<LlmResponseChunk> streamComplete(LlmRequest request) async* {
    try {
      logger.debug('Groq stream request with model: $model');
      ...
      if (streamedResponse.statusCode >= 200 && streamedResponse.statusCode < 300) {
        ...
        final error = 'Groq API Error: ${streamedResponse.statusCode} - $responseBody';
        logger.error(error);

        yield LlmResponseChunk(
          textChunk: 'Error: Unable to get a streaming response from Groq.',
          isDone: true,
          metadata: {'error': error, 'status_code': streamedResponse.statusCode},
        );
      }
    } catch (e, stackTrace) {
      logger.error('Error streaming from Groq API: $e\n$stackTrace');
  ...
  LlmToolCall? extractToolCallFromMetadata(Map<String, dynamic> metadata) {
    if ((metadata.containsKey('tool_call_start') || metadata.containsKey('tool_call_update')) &&
      ...
      final toolId = metadata['tool_call_id'] as String? ??
          'groq_tool_${DateTime.now().millisecondsSinceEpoch}';
  ...
  Future<List<double>> getEmbeddings(String text) async {
    throw UnimplementedError(
      'Groq does not provide embeddings API. Use a dedicated embedding provider.',
    );
  }
  ...
  Future<void> close() async {
    _client.close();
    logger.debug('Groq provider client closed');
  }
...
class GroqProviderFactory implements LlmProviderFactory {
  @override
  ...
  LlmInterface createProvider(LlmConfiguration config) {
    ...
    if (apiKey == null || apiKey.isEmpty) {
      throw StateError('API key is required for Groq provider');
    }

    return GroqProvider(
      apiKey: apiKey,
      model: config.model ?? GroqProvider.defaultModel,
